...
grpc::Status ImageDetectionService::DetectImage(::grpc::ServerContext *context,
    ...
    for (const auto & obj : request->object_to_detect()) {
        std::string obj_lowercase;
        std::transform(obj.begin(), obj.end(), obj_lowercase.begin(), [](unsigned char c){ return std::tolower(c); });
        if (valid_objects.find(obj_lowercase) != valid_objects.end())
            requested_objects.insert(obj_lowercase);
    }
    ...
    VA::FaceEyesDetector face_detector("/usr/share/opencv/haarcascades/haarcascade_frontalface_alt.xml",
                                       "/usr/share/opencv/haarcascades/haarcascade_eye_tree_eyeglasses.xml");
    face_detector.initialize();
    if (!face_detector.is_initialized())
        return grpc::Status(grpc::StatusCode::INTERNAL, "Server could not initialize necessary resources to process this request.");
